...
public class SensorNetworkGraph extends JPanel implements Runnable {
    ...
    private List<Integer> nodesConnectedPath = new ArrayList<>();

    public void setDrawPath(boolean drawPath) {
        this.drawPath = drawPath;
    }

    public void setNodesConnectedPath(List<Integer> nodesPath) {
        this.nodesConnectedPath = nodesPath;
    }
    ...
    public Map<Integer, HashSet<Integer>> getAdjList() {
        return adjList;
    }

    public void setAdjList(Map<Integer, HashSet<Integer>> adjList) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if(drawPath == true) {
            ...
            g2.setStroke(new BasicStroke(2f));
            for (int i = 0; i < nodesConnectedPath.size(); i++) {
                if (!nodesConnectedPath.isEmpty() && ((i + 1) < nodesConnectedPath.size())) {
                    int x1 = graphPoints.get(nodesConnectedPath.get(i) -1).x;
                    int y1 = graphPoints.get(nodesConnectedPath.get(i) -1).y;
                    int x2 = graphPoints.get(nodesConnectedPath.get(i + 1) -1).x;
                    int y2 = graphPoints.get(nodesConnectedPath.get(i + 1) -1).y;
                    g2.drawLine(x1, y1, x2, y2);
                }
            }
        }
